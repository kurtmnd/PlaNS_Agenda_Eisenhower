...
def insert_new_activity():
    ...
    layout = [
[sg.Text('Activity brief description:', size=(18, 1)),
    sg.InputText(size=(32, 1))],
[sg.Frame(layout=[[
    sg.Radio('Planned Task', "RADIO1", default=True, size=(13,1)),
    sg.Radio('Task Done', "RADIO1")]], title=''),
sg.Text("Date:", size=(4, 1), tooltip='Format: YYYY-MM-DD'),
    sg.InputText(size=(12, 1), tooltip='1:Today / 2:Tomorrow / 0:Yesterday')],
[sg.Text('Hour:', size=(4, 1)),
    sg.Slider(range=(0, 24), orientation='h', size=(31, 19), default_value=0, tooltip='0:NotDefined'),
    sg.Checkbox(':30?', size=(3,1))],
[sg.Submit(), sg.Cancel()]
    ]
    ...
    if event=="Submit":
        ...
        new_row.at[0, 'BriefDesc'] = values[0]
        if values[1]: new_row.at[0, 'PlanDone'] = 'Plan'
        else: new_row.at[0, 'PlanDone'] = 'Done'

        aux0 = values[3]
        yesterday = Today - datetime.timedelta(days=1)
        if values[3]=='0': aux0 = date_to_str1(yesterday)
        if values[3]=='1': aux0 = date_to_str1(Today)
        if values[3]=='2': aux0 = date_to_str1(Tomorrow)
        new_row.at[0, 'Date1'] = aux0
        ...
        if h>0:
            ...
            aux2 = ':00'
            if values[5]: aux2 = ':30'
            new_row.at[0, 'Hour1'] = aux1+aux2
        else:
            new_row.at[0, 'Hour1'] = '0'
...
st1 = ScrolledText(centralFrame1, width = ww1, height=hh)
st1.pack(side=LEFT)
st1.configure(font=(fft, ffs), wrap=WRAP_TEXT)

st2 = ScrolledText(centralFrame1, width = ww1, height=hh)
st2.pack(side=LEFT)
...
st3 = ScrolledText(centralFrame2, width = ww2, height=hh)
st3.pack(side=LEFT)
st3.configure(font=(fft, ffs), wrap=WRAP_TEXT)

st4 = ScrolledText(centralFrame2, width = ww2, height=hh)
st4.pack(side=LEFT)
...
ld3 = Label(centralFrame3, text="OTHER", width = ww1+3, relief=RIDGE, bg="whitesmoke").pack(side=TOP)

st5 = ScrolledText(centralFrame3, width = ww3, height=hh, bg='whitesmoke')
st5.pack(side=LEFT)
